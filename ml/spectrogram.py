...
def generate_xai_visualization(audio_path, output_path=None):
    ...
    try:
        ...
        if str(audio_path).lower().endswith('.mp3'):
            d = miniaudio.mp3_read_file_f32(str(audio_path))
        else:
            d = miniaudio.decode_file(str(audio_path), output_format=miniaudio.SampleFormat.FLOAT32, nchannels=2)
        y = np.array(d.samples, dtype=np.float32)
        sr = d.sample_rate
        if d.nchannels > 1:
            y = np.mean(y.reshape(-1, d.nchannels), axis=1)
    
    # Create the figure
    fig, (ax1, ax2) = plt.subplots(nrows=2, figsize=(18, 12), sharex=True)
